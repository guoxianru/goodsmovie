...
def admin_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        urls = [j.url for i in auths for j in auth_list if i == j.id]
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if form.logo.data.filename != '':
            ...
            form.logo.data.save(app.config['LOGO_DIR'] + movie.logo)
...
def preview_add():
    ...
    if form.validate_on_submit():
        ...
        # logo = change_filename(file_logo)
        logo = form.logo.data.filename
        form.logo.data.save(app.config['PREVIEW_DIR'] + logo)
        preview = Preview(
            title=data['title'],
            logo=logo
        )
...
def preview_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if form.logo.data.filename != '':
            ...
            preview.logo = form.logo.data.filename
            form.logo.data.save(app.config['PREVIEW_DIR'] + preview.logo)
...
def comment_del(id):
    ...
    oplog = Oplog(
        admin_id=session['admin_id'],
        ip=request.remote_addr,
        reason='删除一条评论：%s' % comment.user.name
    )
    db.session.add(oplog)
    db.session.commit()
...
def moviecol_del(id):
    ...
    oplog = Oplog(
        admin_id=session['admin_id'],
        ip=request.remote_addr,
        reason='删除一条收藏：%s' % moviecol.user.name
    )
    db.session.add(oplog)
    db.session.commit()
...
def role_edit(id=None):
    ...
    if request.method == 'GET':
        auths = role.auths
        form.auths.data = list(map(int, auths.split(',')))
    if form.validate_on_submit():
        data = form.data
        role.name = data['name']
